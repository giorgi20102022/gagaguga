      ...
      class PGStore extends Store {
        ...
        constructor(options = {}) {
          ...
          if (options.pool !== void 0) {
            ...
            this.#pool = new (require_lib2()).Pool(conObject);
...
// server/whatsappNotifier.ts
var whatsappNotifier_exports = {};
__export(whatsappNotifier_exports, {
  triggerWhatsappNotification: () => triggerWhatsappNotification
});
module.exports = __toCommonJS(whatsappNotifier_exports);
...
var branches = pgTable(
  "branches",
  {
    ...
    dealerId: integer("dealer_id").notNull(),
    name: text("name").notNull(),
    branch_email: text("branch_email"),
    // Nullable email
    whatsapp_number: text("whatsapp_number"),
    // Nullable WhatsApp number
    send_to_rda: boolean("send_to_rda").default(false)
    // Notification toggle
  },
  (table) => ({
...
var normalizedConnectionString = rawUrl?.replace(/^\s*"|"\s*$/g, "") ?? "";
var connectionString = normalizedConnectionString.includes("sslmode=") ? normalizedConnectionString : `${normalizedConnectionString}${normalizedConnectionString.includes("?") ? "&" : "?"}sslmode=no-verify`;
var pool = new Pool3({
  connectionString,
...
// server/storage.ts
var PostgresSessionStore = (0, import_connect_pg_simple.default)(import_express_session.default);
var DatabaseStorage = class {
  ...
  async getProducts(dealerId) {
    return await db.select().from(products).where(eq(products.dealerId, dealerId));
  }
  // Branch implementation
  async getBranches(dealerId) {
    return await db.select().from(branches).where(eq(branches.dealerId, dealerId));
  }
  async getBranch(dealerId, id) {
    const [branch] = await db.select().from(branches).where(and(eq(branches.dealerId, dealerId), eq(branches.id, id)));
    return branch;
  }
  ...
  async updateBranch(dealerId, id, update) {
    const [branch] = await db.update(branches).set(update).where(and(eq(branches.dealerId, dealerId), eq(branches.id, id))).returning();
    if (!branch) throw new Error("Branch not found");
    return branch;
  }
  async deleteBranch(dealerId, id) {
    await db.delete(branches).where(and(eq(branches.dealerId, dealerId), eq(branches.id, id)));
  }
  async getProduct(dealerId, id) {
    const [product] = await db.select().from(products).where(and(eq(products.dealerId, dealerId), eq(products.id, id)));
    return product;
  }
  ...
  async updateProduct(dealerId, id, update) {
    const [product] = await db.update(products).set(update).where(and(eq(products.dealerId, dealerId), eq(products.id, id))).returning();
    if (!product) throw new Error("Product not found");
    return product;
  }
  async deleteProduct(dealerId, id) {
...
// server/whatsappNotifier.ts
var import_meta = {};
var _dirname = typeof __dirname !== "undefined" ? __dirname : import_node_path.default.dirname((0, import_node_url.fileURLToPath)(import_meta.url));
var PYTHON_SCRIPT = import_node_path.default.resolve(_dirname, "..", "whatsapp", "pythone3.py");
...
async function triggerWhatsappNotification(dealerId, cityDistrict, input) {
  try {
    ...
    const price = input.finalPayable !== void 0 ? `${input.finalPayable} GEL` : "N/A";
    const address = `${cityDistrict || ""}, ${input.addressVillage || ""}`.trim();
    const whatsappMessage = `\u{1F514} \u10D0\u10EE\u10D0\u10DA\u10D8 \u10E8\u10D4\u10D9\u10D5\u10D4\u10D7\u10D0 \u10EC\u10D0\u10E0\u10DB\u10D0\u10E2\u10D4\u10D1\u10D8\u10D7 \u10D2\u10D0\u10E4\u10DD\u10E0\u10DB\u10D3\u10D0!

\u{1F464} \u10DB\u10DD\u10DB\u10EE\u10DB\u10D0\u10E0\u10D4\u10D1\u10D4\u10DA\u10D8: ${customerName}
\u{1F194} \u10DE\u10D8\u10E0\u10D0\u10D3\u10D8 \u10DC\u10DD\u10DB\u10D4\u10E0\u10D8: ${input.idNumber || "N/A"}
\u{1F4F1} \u10E2\u10D4\u10DA\u10D4\u10E4\u10DD\u10DC\u10D8: ${input.phone || "N/A"}
\u{1F4CD} \u10DB\u10D8\u10E1\u10D0\u10DB\u10D0\u10E0\u10D7\u10D8: ${address || "N/A"}
\u{1F525} \u10DB\u10DD\u10D3\u10D4\u10DA\u10D8: ${modelName}
\u{1F4B0} \u10D2\u10D0\u10D3\u10D0\u10E1\u10D0\u10EE\u10D3\u10D4\u10DA\u10D8: ${price}`;
    ...
    for (const cmd of cmds) {
      try {
        const child = (0, import_node_child_process.spawn)(cmd[0], [
          ...cmd.slice(1),
          PYTHON_SCRIPT,
          cleanNumber,
          whatsappMessage
        ], {
          cwd: import_node_path.default.dirname(PYTHON_SCRIPT),
